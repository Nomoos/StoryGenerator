...
def example_1_improve_single_title():
    ...
    if best_score['is_original']:
        ...
        improvement = ((best_score['overall_score'] - all_scores[0]['overall_score']) / 
                      all_scores[0]['overall_score'] * 100)
...
def example_2_batch_analysis():
    ...
    for title in titles:
        # Generate variants
        variants = title_improve.generate_title_variants(
            title, segment, age, llm_config, count=3
        )
    ...
    for r in results:
        ...
        print(f"{r['original']:<35} {r['original_score']:>6.1f}  {r['best_score']:>6.1f}  {change_icon:>5}")
...
def example_3_compare_strategies():
    ...
    for i, variant in enumerate(local_variants, 1):
        ...
        improvement = ((score['overall_score'] - original_score['overall_score']) / 
                      original_score['overall_score'] * 100)
        print(f"{i}. {variant}")
        ...
        if registry['titles']:
            ...
            for key, data in sorted_titles[:5]:
                if data['is_changed']:
                    ...
                    print(f"    Score: {data['original_score']:.1f} → {data['improved_score']:.1f} ({data['improvement_pct']:+.1f}%)")
    else:
