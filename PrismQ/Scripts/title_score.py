...
def score_title_locally(title: str, gender: str, age: str, config: Dict) -> Dict:
    ...
    if age_start < 24:
        if any(
            word in word_lower
            for word in ["tiktok", "trend", "viral", "challenge", "life", "friend"]
        ):
            relevance_score += 20
    ...
    # Older audiences (24+) prefer practical, life content
        ...
        if any(
            word in word_lower for word in ["career", "money", "health", "relationship", "life"]
        ):
            relevance_score += 20
    ...
    if gender == "women":
        if any(
            word in word_lower
            for word in ["beauty", "style", "wellness", "self-care", "relationship"]
        ):
            relevance_score += 10
    elif gender == "men":
    ...
    if any(
        word in word_lower for word in ["everyone", "nobody", "never", "always", "worst", "best"]
    ):
        viral_score += 15
    ...
    return {
        ...
        "rationale": rationale,
        "voice_recommendation": {"gender": voice_gender, "reasoning": voice_reasoning},
    }
...
def recommend_voice(title: str, target_gender: str, age: str) -> str:
    ...
    if any(
        word in title_lower
        for word in ["mystery", "secret", "dark", "horror", "scary", "hidden", "truth"]
    ):
        return "M"
...
def generate_voice_reasoning(
    title: str, target_gender: str, age: str, recommended_gender: str
) -> str:
    """
...
def score_titles_for_segment(
    titles_path: Path, scores_path: Path, voices_path: Path, gender: str, age: str, config: Dict
) -> Tuple[int, int]:
    """
    ...
    for file_path in title_files:
        ...
        result = {
            ...
            "target_audience": {"gender": gender, "age": age},
...
def process_all_segments(
    base_path: str = None, audience_config: Dict = None, scoring_config: Dict = None
) -> None:
    """
    ...
    for gender in genders:
        for age_group in age_groups:
            scored, top = score_titles_for_segment(
                titles_path, scores_path, voices_path, gender, age_group, scoring_config
            )
            total_scored += scored
            total_top += top
...
def main():
    ...
    if len(sys.argv) > 1:
        ...
        if len(sys.argv) >= 3:
            ...
            scored, top = score_titles_for_segment(
                titles_path, scores_path, voices_path, gender, age, scoring_config
            )
