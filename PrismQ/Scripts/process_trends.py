...
def process_trends_directory(trends_dir, output_dir, min_value=50):
    ...
    if countries:
        for country in countries:
            country_trends = [
                t for t in aggregated if any(d.get("geo") == country for d in t.get("details", []))
            ]
            if country_trends:
                country_suggestions = generate_content_suggestions(
                    country_trends, max_suggestions=20
                )
