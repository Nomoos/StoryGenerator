...
def deduplicate_content(content_items: List[Dict]) -> Tuple[List[Dict], Dict]:
    ...
    report = {
        ...
        "retention_rate": round(len(unique_items) / len(content_items) * 100, 2) if content_items else 0
    }
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Deduplicate content from quality-scored stories"
    )
    parser.add_argument(
        "--segment", 
        choices=["women", "men"],
        help="Process specific gender segment"
    )
    parser.add_argument(
        "--age",
        choices=["10-13", "14-17", "18-23"],
        help="Process specific age bucket"
    )
    parser.add_argument(
        "--date",
    ...
    if args.segment and args.age:
        ...
        segments = [
            ("women", "10-13"), ("women", "14-17"), ("women", "18-23"),
            ("men", "10-13"), ("men", "14-17"), ("men", "18-23")
        ]
    else:
    ...
    for gender, age_bucket in segments:
        ...
        if result["status"] == "success":
            ...
            print(f"   📉 Removed {result['total_duplicates']} duplicates ({100-result['retention_rate']:.1f}%)")
