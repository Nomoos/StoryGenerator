...
Examples:
  ...
  python alt_sources_scraper.py --sources twitter --gender men --age 14-17 --topic "gaming"
    ...
    parser.add_argument(
        "--all-demographics",
        action="store_true",
        help="Scrape all gender/age combinations"
    )
...
def run_scraping(scrapers: Dict, 
    ...
    print(f"\n{'='*60}")
    print(f"Starting scraping: {len(scrapers)} sources × {len(demographics)} demographics = {total} tasks")
    print(f"{'='*60}\n")
