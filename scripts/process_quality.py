...
def load_config():
    ...
    try:
        ...
        return {
            "quality_thresholds": {"min_score": 70, "reprocess_score": 50, "underscore_prefix": "_"}
        }
...
def assess_emotional_impact(content_data, content_text, content_type):
    ...
    if genre in ["drama", "romance", "thriller", "horror"] or category in [
        "human interest",
        "emotional",
    ]:
        score += 15
...
def calculate_score(content_data, scoring_config=None):
    ...
    if all(
        metric in content_data for metric in ["novelty", "emotional", "clarity", "replay", "share"]
    ):
        # Use existing scores
        metric_scores = {
    ...
    viral_weights = scoring_config.get(
        "viral",
        {"novelty": 0.25, "emotional": 0.25, "clarity": 0.20, "replay": 0.15, "share": 0.15},
    )
