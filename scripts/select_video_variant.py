...
def batch_select(config_path: str, output_dir: str = None, save_reports: bool = True):
    ...
    try:
        results = selector.batch_select_variants(
            variant_groups=variant_groups, output_dir=output_dir, save_reports=save_reports
        )

        # Save batch results
        if save_reports:
            batch_report_path = os.path.join(
                output_dir or os.path.dirname(config_path), "batch_variant_selection.json"
            )
            ...
            batch_report = {
                ...
                "selections": {
                    shot_id: {
                        ...
                        "overall_score": (
                            report["selected_score"]["overall_score"]
                            if report["selected_score"]
                            else None
                        ),
                    }
...
Examples:
  ...
  python scripts/select_video_variant.py --output-dir /path/to/reports v1.mp4 v2.mp4
    ...
    parser.add_argument("variants", nargs="*", help="Paths to video variant files")

    parser.add_argument("--shot-id", dest="shot_id", help="Shot ID for the selection report")
    ...
    parser.add_argument(
        "--output-dir", dest="output_dir", help="Directory to save selection reports"
    )

    parser.add_argument("--batch", dest="batch_config", help="Batch process from JSON config file")
    ...
    try:
        if args.batch_config:
            ...
            if len(args.variants) < 2:
                print(
                    "⚠️  Warning: Only one variant provided. Selection requires at least 2 variants."
                )
