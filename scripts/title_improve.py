...
Generate exactly {count} title variants, one per line, numbered 1-{count}."""
        ...
        response = client.chat.completions.create(
            model=model,
            messages=[
                {"role": "system", "content": "You are an expert at creating viral social media titles."},
...
def improve_title(
    ...
    if all_scores[0]['overall_score'] > 0:
        improvement_pct = ((best_score['overall_score'] - all_scores[0]['overall_score']) / 
...
def main():
    ...
    parser.add_argument('--variant-count', type=int, default=5, help='Number of variants to generate (default: 5)')
    ...
    if all_results:
        ...
        print(f"Average improvement: {sum(r['best_title']['improvement_pct'] for r in all_results) / len(all_results):.1f}%")
    else:
