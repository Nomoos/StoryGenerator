...
namespace StoryGenerator.Research
{
    ...
    // Extension methods for Orchestrator
    public static class OrchestratorExtensions
    {
        /// <summary>
        /// Generate a script from a title and prompt.
        /// </summary>
        public static async Task<string> GenerateScriptAsync(
            ...
            CancellationToken cancellationToken = default)
        {
            // Implementation would use the LLM client
            // For prototype, return a placeholder
            return $"# {title}\n\n{prompt}\n\n[Script content would be generated here]";
        }

        /// <summary>
        /// Transcribe and normalize audio in one operation.
        /// </summary>
        public static async Task<AudioProcessingResult> TranscribeAndNormalizeAsync(
            this Orchestrator orchestrator,
        ...
        /// <summary>
        /// Process a video file with cropping and encoding.
        /// </summary>
        public static async Task<VideoProcessingResult> ProcessVideoAsync(
            this Orchestrator orchestrator,
            string inputPath,
            string outputPath,
            CancellationToken cancellationToken = default)
        ...
        /// </summary>
        public static async Task<List<string>> GenerateIdeasAsync(
            this Orchestrator orchestrator,
            string topic,
            int count = 20,
            CancellationToken cancellationToken = default)
        {
            // For prototype, generate placeholder ideas
            var ideas = new List<string>();
            for (int i = 1; i <= count; i++)
            {
                ideas.Add($"Idea {i}: {topic} - concept variation {i}");
            }
            return ideas;
        }
        ...
        /// </summary>
        public static async Task<TitleScore> ScoreTitleAsync(
            this Orchestrator orchestrator,
            string title,
            CancellationToken cancellationToken = default)
        {
            // For prototype, return sample scores
            return new TitleScore
            {
                ...
                Replay = 13,
                Share = 12,
                Rationale = $"Title '{title}' shows strong viral potential."
            };
        }
    }
}
